...
def _build_dome(audit: dict) -> dict:
    """A DOME (Data / Optimization / Model / Evaluation) methods block — the
    structured ML-methods summary recommended for biology (Walsh et al.,
    Nature Methods 18:1122-1127, 2021). Mostly a crosswalk of existing fields
    plus explicit limitations."""
    ds = audit.get("dataset", {}); meta = audit.get("meta", {}); cfg = audit.get("config", {})
    models = audit.get("models", {}); util = audit.get("utility", {})
    cvc = cfg.get("cv", {})
    return {
        "data": {
        ...
        "model": {
            "integration_regimes": "early (feature concatenation), intermediate (masked "
                                   "global-pooling neural network), late (stacking)",
            "reference_estimator": models.get("reference_estimator"),
            "primary_metric": models.get("primary_metric"),
        },
        "evaluation": {
            "cross_validation": f"group-aware nested {cvc.get('n_splits')}-fold cross-validation",
            "uncertainty": f"group-level bootstrap 95% confidence intervals "
            ...
            "limitations": [
                "Internal group-aware cross-validation only; no external/independent validation.",
                "No hyperparameter tuning.",
                "Permutation importance is unconditional (biased under feature collinearity).",
...
def _model_card_md(audit: dict) -> str:
    ...
    best = util.get("best_model", {})
    mods = ", ".join(f"{m['name']} ({m['n_features']})" for m in ds.get("modalities", []))
    lines = [
        f"# Model card — {meta.get('run_name', 'omicau audit')}",
        "",
        "## Intended use",
        ...
        f"{best.get('primary')}",
        "- Group-aware nested cross-validation; every metric reported with a group-level "
        "bootstrap 95% confidence interval.",
        "- Leakage controls (shuffled target/features/noise); calibration for binary classification.",
        "",
        "## Caveats and limitations",
        "- No external validation and no hyperparameter tuning.",
        "- Permutation importance is unconditional and biased under feature collinearity.",
        "- Predicted probabilities may be miscalibrated (class-weight balancing).",
        "- Research use only.",
        "",
        f"Generated by omicau v{meta.get('tool_version', '')} · "
...
<div id="research" class="panel">
  <section>
    ...
    <div class="consequence"><strong>Cross-site stress test.</strong> Blocking folds on batch (a new-batch
      ...
      {{ '%+.3f'|format(util.batch_blocked.optimism_gap) }}{% if util.batch_blocked.optimism_gap and util.batch_blocked.optimism_gap > 0.05 %}; the standard CV overstates performance on an unseen site{% endif %}.</div>
    {% endif %}
  </section>
    ...
    <details class="glossary">
      ...
      <div class="gloss-body">
        ...
        <ul class="flags">
          ...
          <li><span class="mono">{{ k }}</span>: {% if v is mapping %}{% for kk, vv in v.items() %}{{ kk }}: {{ vv }}{% if not loop.last %}, {% endif %}{% endfor %}{% elif v is iterable and v is not string %}{{ v|join('; ') }}{% else %}{{ v }}{% endif %}</li>
          {% endfor %}
        </ul>
        {% endfor %}
        <p class="note-line">A <code>dome</code> block and a <code>MODEL_CARD.md</code> are also written alongside <code>audit.json</code>.</p>
      </div>
    </details>
    {% endif %}
  </section>
