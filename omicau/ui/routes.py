...
def register(app) -> None:  # noqa: C901 - a flat set of small handlers
    ...
    async def clinical_columns(sid: str):
        return I.inspect_clinical(_clinical_path(_sess(sid)))
    ...
    async def consequence(sid: str, column: str, kind: str, task: str = "auto",
        ...
        if kind == "target":
            return I.target_consequence(path, column, task)
        if kind == "group":
            return I.group_consequence(path, column)
        if kind == "batch":
            return I.batch_consequence(path, column, target)
    ...
    async def align(sid: str):
        ...
        return I.alignment_preview(_modalities(s), _clinical_path(s), cm.get("sample_id"))
    ...
    async def progress(sid: str):
        ...
        return {"status": r["status"], "stages": r.get("stages", []),
                "error": r.get("error"), "report_ready": bool(r.get("report")),
